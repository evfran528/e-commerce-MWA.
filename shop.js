/* ============================================================
   ...
   (kept independent from home.js, which assumes homepage-only
   elements like the carousel and would throw on this page)
   ============================================================ */
...
(function initHeroSlider() {
    ...
    if (!slidesWrap) return; // nothing to do if the hero isn't on this page
    ...
    // touch swipe support (left/right) for mobile — arrows hide on very
    // small screens, so this is the fallback way to navigate manually
...
// Generic size run used by the quick-view picker. Swap per-product if you
// later want different runs for e.g. dresses vs. shorts.
...
// cart is no longer page-local — it lives in CartStore (cart.js),
// which persists it in localStorage so home.js / aboutus.js / shop.js
// all see the same cart.

// the product currently open in the quick-view modal, and the size picked
...
function buildCard(product) {
    ...
    // Clicking "Add to Cart" on a card opens the quick-view / size picker
    // instead of adding straight away.
...
/* ============================================================
   CART  (backed by the shared CartStore — see cart.js — so it
   stays in sync with home.js and aboutus.js)
   ============================================================ */
...
// keep the modal in sync if the cart changes from elsewhere (e.g. another
// tab, or the header badge logic in cart.js)
...
checkoutForm.addEventListener("submit", (e) => {
    ...
    // In a real store this is where you'd send the order to a backend / API.
...
/* ============================================================
   URL PARAMS (pre-filter from mega-menu links)
   ============================================================ */
